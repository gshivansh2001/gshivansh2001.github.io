...
While working on an SDID specification for a recent project, I encountered an unexpected problem: the result for `sdid` analysis only gives me the ATT value and does not let me know of the estimates of the coefficients, I knew about getting some information on the coefficients or covariates using `e(beta)` after running `sdid` but did not match the coefficients I obtained from a corresponding fixed-effects regression using `reghdfe`. The ATT estimates were stable, but the covariate estimates were not only numerically different—they sometimes differed by orders of magnitude.
...
```stata
sdid l_wo state date treat, vce(bootstrap) method(sdid) covariates(l_pop l_black l_unemp l_inc l_HS l_college lgdp, optimized)
```
...
```stata
reghdfe l_wo l_pop l_black l_unemp l_inc l_HS l_college lgdp if W == 0, absorb(state date) cluster(state)
```
...
- <a href="https://www.statalist.org/forums/forum/general-stata-discussion/general/1758770-sdid-obtain-the-coefficients-and-std-err-of-covariates">Statalist thread (2024): *SDID: Obtain the coefficients and Std. Err. of covariates*  </a>
- <a href="https://www.statalist.org/forums/forum/general-stata-discussion/general/1718121-how-to-discern-the-coefficients-and-their-p-values-of-covariates-using-sdid">Statalist thread (2023): *How to discern coefficients and their p-values of covariates using sdid*</a>
